...
`e_ident[EI_VERSION]` 给出了 ELF 头的版本号。目前这个值必须是`EV_CURRENT`，即之前已经给出的`e_version`。
...
程序头部的虚拟地址可能并不是程序内存镜像中实际的虚拟地址。可加载段中`p_vaddr`最小的内存虚拟地址对应的实际虚拟地址叫基地址，也就是程序实际在虚拟内存中的起始地址。基地址的一个用途就是在动态链接期间重新定位程序。
...
要计算基地址，首先要确定可加载段中 p_vaddr 最小的内存虚拟地址，之后把该内存虚拟地址`截断`为最接近的最大页面大小的倍数，获得基地址。根据要加载到内存中的文件的类型，内存地址可能与 p_vaddr 相同也可能不同。
